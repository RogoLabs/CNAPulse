...
async function loadCNADetail() {
  ...
  try {
    ...
    // Try per-CNA detail file first (lighter, has timeline)
    ...
    if (detailResponse.ok) {
      ...
      // Fallback to full anomaly_data.json
...
function displayCNA(cna) {
  ...
  if (cna.cna_org_name) {
    document.getElementById("cna-subtitle").textContent =
      `CNA ID: ${cna.cna_name}`;
  } else {
...
function renderTimelineChart(cna) {
  ...
  if (!cna.timeline_13months || cna.timeline_13months.length === 0) {
    ctx.parentElement.innerHTML =
      '<p class="text-gray-500 text-center py-12">No timeline data available</p>';
    return;
  }
          ...
          callbacks: {
            label: function (context) {
              ...
              if (datasetLabel === "12-Month Average") {
                ...
                const label = isCurrent[context.dataIndex]
